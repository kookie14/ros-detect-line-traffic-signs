...
model_path = "/home/thong/my_catkin/src/self-driving-car/src/Traffic.h5"
...
def callback_processing_thread(proc_image):
    ...
    if sign_flag == 1:
        command_pub.publish(String("STOP"))
    elif sign_flag == 2:
        command_pub.publish(String("TURN_RIGHT"))
    elif sign_flag == 3:
        command_pub.publish(String("TURN_LEFT"))
    elif sign_flag == 4:
        command_pub.publish(String("GO_STRAIGHT"))
    else:
        pass
    sign_flag = 0
...
def sign_command_callback(data):
    ...
    if data.data == "STOP":
        ...
        while((rospy.get_time() - start) < duration.to_sec()):
            steeting_angle_pub.publish(Float64(-0.475))
        velocity_pub.publish(Float64(0))
        command_sign = 0
